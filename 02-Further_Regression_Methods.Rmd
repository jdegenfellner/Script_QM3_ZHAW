...
you can (sometimes) use [**Poisson regression**](https://en.wikipedia.org/wiki/Poisson_regression).
...
($log(\lambda) = 0.5x - 2$) **the log of the mean is a linear function of** $x$ 
...
- The predictor is usually centered (as is recommended in *Rethinking*)
  for easier interpretation of the prior and probably numeric stability,
  but for demonstrative purposes we skip it to make the results directly 
  comparable with the Frequentist result.
...
Since Poisson regression is a generalized linear model (with log as link function), 
...
We ignore the `deviance` for now. The math is not that complicated,
