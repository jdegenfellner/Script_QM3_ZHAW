...
- `Coefficients` are the estimated parameters of the model **on the logit scale**.
  **Interpretation**: Two participants with an expected difference of $1$ year in their age,
...
Let's present the results nicely on the odds ratio scale and 
we also add confidence intervals (CIs) for the odds ratios,
which can be conveniently calculated using R:
...
- At $AGE=0$, the odds for our event (maybe heart attack) are $0.891$.
...
- Increasing the `AGE` by $1$ year increases the odds of a $1$ by a factor of 
  $4.58$.
...
with the actual outcomes. This is called a **confusion table**.
...
We need to compare this number with the baseline accurary.
If $90%$ of the observations are $0$s, then we would have an accuracy of $90\%$,
...
**Model checks** are also interesting. 

Here is a list of assumptions for this type of model:
...
Let's see what `check_model` has to say:
...
In [exercise 14](exercise14_LMM), we will look at the model checks in more detail.
