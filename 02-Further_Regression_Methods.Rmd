...
- [Linear relationship](https://www.sthda.com/english/articles/36-classification-methods-essentials/148-logistic-regression-assumptions-and-diagnostics-in-r/) 
  ...
  Normally, this plot is not quite so perfect:
  ```{r}
   library(tidyverse)
   pred_df <- data.frame(
   AGE = AGE,
   logit_hat = predict(modlog, 
                       type = "link")  # linear predictor η = β0 + β1 * AGE
   )
   ggplot(pred_df, aes(x = AGE, y = logit_hat)) +
    geom_point(color = "blue", alpha = 0.6) +
    geom_smooth(method = "loess", se = FALSE, color = "red", lwd = 1.2) +
    labs(
      title = "Checking linearity in the logit: logit(p) vs. AGE",
      x = "AGE (standardized)",
      y = "Logit(p) = Linear predictor"
    ) +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5))
  ```
