...
We want to replicate the main result of the paper 
...
- The primary outcome is the variable *headache frequency*
  ...
  We do this, since they as well used a version of this test.
...
- $i = 1, \ldots, 99$ are the $99$ participants
...
Let's first bring our data in shape and (for simplicity) kill all rows with missing values.
...
is only in one group (of the three) and they do not crossover.
...
Residual variance is high compared to this.
...
Important for us are the so-called **contrasts**. 
This gives us the model-estimated differences in (expected) headache frequency between 
the groups at each time point.

Bonferroni correction is used to adjust the $p$-values for multiple comparisons
...
are compared. The mean difference is $-3 (-3 \text{ to } -2)$ days/month in Table 3. 
In our `lmer` output, we find the row
`(MMT+ex) - (Sham+ex)  -2.6892 0.257 260   -3.308   -2.070`.
Rounding to integers results in the same result.

If you want to see the **random effects** of all $91$ participants:
...
#### Bayesian attempt to replicate the results

We can also try to fit the model using the `rethinking` package.

Model equations and priors:

TODO - adapt model... beta_1[group]...3 different betas...

\[
\begin{array}{rcl}
\text{headache frequency}_{ij} &\sim& \text{Normal}(\mu_{ij}, \sigma) \\
\mu_{ij} &=& \alpha + \beta_1 \text{group}_i + \beta_2 \text{time}_j + 
              \beta_3 (\text{group}_i \times \text{time}_j) + u_i + \varepsilon_{ij}\\
u_i &\sim& \text{Normal}(0, \sigma_u) \\
\sigma_u &\sim& \text{Exponential}(1) \\
\alpha &\sim& \text{Normal}(5, 1.5) \\
\beta_1 &\sim& \text{Normal}(0, 1.5) \\
\beta_2 &\sim& \text{Normal}(0, 1.5) \\
\beta_3 &\sim& \text{Normal}(0, 1.5) \\ 
\varepsilon_{ij} &\sim& \text{Normal}(0, \sigma) \\
\end{array}
\]

The prior for $\beta_3$ is hard since there is no natural meaning to it.
