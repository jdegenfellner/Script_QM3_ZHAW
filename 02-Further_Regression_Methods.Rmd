...
**does not make much sense** here. The outcome can only be $0$ or $1$ but
the regression line would estimate all values in between plus
values outside of this range, which is not possible.

We therefore bend our reality a bit and **model not the expected outcome but the expected probability of a $1$**.
...
- `Coefficients` are the estimated parameters of the model **on the logit scale**.
  ...
  **Interpretation**: Two participants with a difference of $1$ year in their age,
  have an expected difference of $\beta_1 = 1.20158$ in the log-odds of having 
  a heart attack. 
...
- For very large or very small values of `AGE`, the probability of 
  $Y_i = 1$ approaches $1$ or $0$, respectively. So this works nicely to describe
...
with the actual outcomes. This is called a **confusion table** (or [confusion matrix](https://en.wikipedia.org/wiki/Confusion_matrix)).
...
up or down in order to improve performance.
...
- Independent observations (which we have, since they were constructed this way).
  Obervations might not be independent in time series data or repeated measurements of
...
- No perfect separation: This means that there should not be a *single* cutoff under 
  ...
  R will throw an error if this is the case.
...
assumes. We also assumed the variability ($\sigma$) around the mean ($\mu$) increased with the mean,
...
you can (sometimes) use the [**Poisson regression**](https://en.wikipedia.org/wiki/Poisson_regression).
...
The above equation implies that the variance increases with the mean!
...
We want to save time and quickly create simulated data.
...
- In dark red, I drew the function $\lambda(x) = e^{0.5x - 2}$, 
  which is the mean and the variance of the Poisson distribution.
- Both (mean and variance) are equal and clearly increase with $x$.

As you can see (in the code) from the definition of the parameter $\lambda$:
$log(\lambda) = 0.5x - 2$. So **the log of the mean is a linear function of** $x$ 
...
Count data cannot be negative, hence we use the log to ensure 
that $\lambda$ is always positive: $exp(0.5x - 2) > 0~\forall x$.
...
\log(\lambda_i) &=& \alpha + \beta (x_i - x) \\
\end{array}
\]

- $\lambda_i$ is the expected value of $Y_i$ for observation $i$.
- The predictor is centered again (as is recommended in *Rethinking*).
...
prior predictive distributions. According to the book, the flat priors for $\alpha$ and $\beta$ 
...
Since Poisson regression is a generalized linear model, we can use the `glm` function in R:
```{r, echo = TRUE}
...
The coefficients are very similar to the Bayesian model and seem to be closer to the true values.
The reason is probably the very rough priors we used in the Bayesian model.
