...
Correlation Coefficients (ICCs)}. The are called \emph{random} intercept
since they are drawn from a common normal distribution, of which the
...
\item
  ...
  \emph{clustered} within the subject. They are not independent. Hence,
  the classical linear regression model is not appropriate and would
...
would then model the survival of each individual tadpole (\(1\) could
mean \emph{survived} and \(0\) could mean \emph{died}). Formally, this
would be a so-called
\href{https://en.wikipedia.org/wiki/Logistic_regression}{logistic
regression}, since our outcome is binary (\emph{survided}/\emph{died}).
...
\textbf{regularizing prior} for \(\alpha_j\) is a normal distribution
with mean \(0\) and standard deviation \(1.5\).

Technically, we could use different priors for each intercept, but this
would be a bit cumbersome and we do not have useful prior information
...
posterior means and standard deviations of our \(48\) interecepts
(\(48\) tanks with tadpoles). These intercepts are on the scale
\((-\infty, \infty)\).
...
for the \texttt{a{[}tank{]}} parameters. This model learns from the data
how much the intercepts should be pulled towards a common mean. Now we
have an additional level in the model hierarchy:
...
## Chain 4 Exception: normal_lpdf: Scale parameter is 0, but must be positive! (in '/var/folders/pm/jd6n6gj10371_bml1gh8sc5w0000gn/T/RtmpHHmI6Y/model-14a4f9d02f54.stan', line 15, column 4 to column 32)
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-5-1.pdf}}
...
In a \textbf{prediciton context} we ask ourselves: How well do the
models \emph{predict} the data (which was measured using residuals
\(y_i - \hat{y}_i\) in regression)?

There are many nuances but the \textbf{main paradigm} (also in classical
machine learning) is to fit the model on a \textbf{training set} and
evaluate the model on a \textbf{test set}. Should you be interested in
the (admittedly very technical) details, I can recommend the book
\emph{Elements of Statistical Learning} by Hastie, Tibshirani and
Friedman (free
\href{https://www.sas.upenn.edu/~fdiebold/NoHesitations/BookAdvanced.pdf}{here}).

During training, the model tries to learn the underlying structure of
the data. The test set is used to evaluate the model's performance on
unseen data. This is the ultimate and brutally honest test of the
...
height prediction example in QM2}, we would fit the model on 80\% of
...
\item
  ...
  difference in WAIC between the models (\(15.88993\)). \textbf{Here,
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-7-1.pdf}}
...
\(1.568\) which schould be somewhat similar to the \texttt{sigma} in the
...
\item
  ...
  (\(4-9\)) at week \(0\) and can therefore not be normally distributed
...
\NormalTok{  ) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}
    ...
    \AttributeTok{time =} \FunctionTok{recode}\NormalTok{(time,}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-10-1.pdf}}
...
frequency between the groups at each time point.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# constrasts:}
\NormalTok{emm }\OtherTok{\textless{}{-}} \FunctionTok{emmeans}\NormalTok{(model, }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Group }\SpecialCharTok{|}\NormalTok{ time\_f)  }\CommentTok{\#}
\FunctionTok{summary}\NormalTok{(}\FunctionTok{contrast}\NormalTok{(emm, }\AttributeTok{method =} \StringTok{"pairwise"}\NormalTok{,}
                \AttributeTok{adjust =} \StringTok{"bonferroni"}\NormalTok{),}
                \AttributeTok{infer =} \FunctionTok{c}\NormalTok{(}\ConstantTok{TRUE}\NormalTok{, }\ConstantTok{FALSE}\NormalTok{))  }\CommentTok{\# TRUE for CI and FALSE for p{-}values}
\end{Highlighting}
\end{Shaded}
...
\item
  In our \texttt{lmer} output, we find the row
  \texttt{(MMT+ex)\ -\ (Sham+ex)\ \ -2.6892\ 0.257\ 260\ \ \ -3.308\ \ \ -2.070}.
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-14-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-14-2.pdf}}
...
\begin{verbatim}
## Chain 1 Exception: normal_lpdf: Scale parameter is 0, but must be positive! (in '/var/folders/pm/jd6n6gj10371_bml1gh8sc5w0000gn/T/RtmpHHmI6Y/model-14a4f3f0ada79.stan', line 29, column 4 to column 31)
\end{verbatim}
...
and credible interval) of the treatment effect of \emph{MMT+ex} compared
to \emph{Sham+ex} at \emph{week \(4\)}.

For this, we calculate the distribution of group \(2\) (\emph{MMT+ex})
minus group \(3\) (\emph{Sham+ex}) at week \(4\). Then, we calculate the
correct interaction index to give us the required difference. Finally,
we add up the two posterior distributions to get the posterior
distribution of the treatment effect. Time does not vary since we are
only interested in week \(4\).

The result is rather similar to the Frequentist model.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# mean treatment effect: week4,  (MMT+ex) {-} (Sham+ex){-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{post }\OtherTok{\textless{}{-}} \FunctionTok{extract.samples}\NormalTok{(m)}
\CommentTok{\#lapply(post, head)}

\CommentTok{\# MMT+ex (group = 2), time = 2 → index = (2{-}1)×4 + 2 = 6}
\CommentTok{\# Sham+ex (group = 3), time = 2 → index = (3{-}1)×4 + 2 = 10}

\NormalTok{group\_diff }\OtherTok{\textless{}{-}}\NormalTok{ post}\SpecialCharTok{$}\NormalTok{beta\_group[,}\DecValTok{2}\NormalTok{] }\SpecialCharTok{{-}}\NormalTok{ post}\SpecialCharTok{$}\NormalTok{beta\_group[,}\DecValTok{3}\NormalTok{]}
\NormalTok{interaction\_diff }\OtherTok{\textless{}{-}}\NormalTok{ post}\SpecialCharTok{$}\NormalTok{beta\_interaction[,}\DecValTok{6}\NormalTok{] }\SpecialCharTok{{-}}\NormalTok{ post}\SpecialCharTok{$}\NormalTok{beta\_interaction[,}\DecValTok{10}\NormalTok{]}
\NormalTok{treatment\_diff\_week4 }\OtherTok{\textless{}{-}}\NormalTok{ group\_diff }\SpecialCharTok{+}\NormalTok{ interaction\_diff}
\FunctionTok{precis}\NormalTok{(}\FunctionTok{data.frame}\NormalTok{(treatment\_diff\_week4), }\AttributeTok{prob =} \FloatTok{0.95}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##                          mean        sd      2.5%     97.5%  histogram
## treatment_diff_week4 -2.68054 0.2522168 -3.190296 -2.191346 ▁▁▁▂▅▇▅▂▁▁
\end{verbatim}
...
content of the outcome variable, which can only take two values.
...
\NormalTok{AGE }\OtherTok{\textless{}{-}} \FunctionTok{rnorm}\NormalTok{(}\DecValTok{100}\NormalTok{)}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-17-1.pdf}}
...
We therefore bend our reality a bit and \textbf{model not the expected
outcome but the expected probability of a \(1\)}. Of course, then we
have the next problem, if we try to model the probability of a \(1\)
...
\begin{itemize}
\item
  \texttt{glm}stands for \emph{generalized linear model}.
\item
  \texttt{family\ =\ binomial(link\ =\ "logit")} specifies that we want
  to use the binomial distribution with the logit link function. In
  generalized linear models, the link function is used to connect the
  linear predictor (the right side of the equation) to the expected
  value of the response variable: In our case, the expected value of a
  ...
  variable (which is binary) is the probability of a \(1\): \(p_i\). In
  general: \[\mathbb{E}(Y|X) = g^{-1}(X \beta)\] Think back to
...
\item
  ...
  year in their age, have an expected difference of
  \(\beta_1 = 1.20158\) in the log-odds of having a heart attack.
  ...
  So, on the odds scale, you multiply the odds for a \(1\) by
  \(e^{\beta_1}\).
...
But, we do not need to do all this by hand, we have R. We can simply
calculate the probabilities estimated by data:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{predicted\_probabilities }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(modlog, }\AttributeTok{type =} \StringTok{"response"}\NormalTok{)}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-19-1.pdf}}
...
in this case) is \emph{associated} (let's not say ``influenced'') with
higher probability of a \(1\).
...
\item
  ...
  of \(Y_i = 1\) approaches \(1\) or \(0\), respectively. So this works
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-22-1.pdf}}
...
maximally wide, since above \(\approx 1\) and below \(\approx -1\) there
are only \(Y_i = 1\) or \(Y_i = 0\) observations which gives some
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-24-1.pdf}}
...
with the actual outcomes. This is called a \textbf{confusion table} (or
...
We could introduce the threshold of \(0.5\) as the cutoff for a \(1\).
If the predicted probability is above \(0.5\), we predict a \(1\),
otherwise a \(0\). Probably, one wants to move this threshold up or down
...
with \texttt{AGE}. The variance of a Bernoulli distribution is
\(p_i(1-p_i)\), which is not constant, if the \(p_i\) is not constant.
The variance is highest at \(p_i = 0.5\) and lowest at \(p_i = 0\) or
\(p_i = 1\). You see this immediately when you plot the variance
function \(p_i(1-p_i)\):
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-26-1.pdf}}
...
\item
  ...
  this way). Obervations might not be independent in time series data or
  repeated measurements of the same person, or clustered data (e.g.,
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(performance)}
\FunctionTok{check\_model}\NormalTok{(modlog)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-27-1.pdf}}
...
\item
  \ldots.distributional check for residuals necessary\ldots??
\item
  ...
  mini example, observations are of course independent.
\end{itemize}
...
\NormalTok{poisson\_data}\SpecialCharTok{$}\NormalTok{lambda }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(poisson\_data}\SpecialCharTok{$}\NormalTok{lambda,}
                              \AttributeTok{labels =} \FunctionTok{paste0}\NormalTok{(}\StringTok{"λ = "}\NormalTok{, lambda\_values))}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-28-1.pdf}}

A Poisson-distributed variable can only take integer values which are of
course non-negative.
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-29-1.pdf}}
...
\item
  ...
  and the variance of the Poisson distribution.
\item
...
\(\lambda\) is always positive: \(exp(0.5x - 2) > 0~\forall x\).
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-31-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-32-1.pdf}}
...
We ignore the \texttt{deviance} for now. The math is not that
complicated, but it is not so important for us at this point.
...
\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-34-1.pdf}}
...
Overdispersion occurs if the variance is larger than the mean.
Underdispersion occurs if the variance is smaller than the mean.

Nice enough, the \texttt{performance} package has a function to check
this:
...
\paragraph{\texorpdfstring{Linearity of the log of
\(\lambda\)}{Linearity of the log of \textbackslash lambda}}\label{linearity-of-the-log-of-lambda}

The following plot should not be perfectly linear if we do not
artificially create the data. We have defined the linear relationship
explicitely, hence the plot is perfectly linear
(\(log(\lambda) = 0.5x-2\)).

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plot}\NormalTok{(x, }\FunctionTok{predict}\NormalTok{(mod\_glm, }\AttributeTok{type =} \StringTok{"link"}\NormalTok{),}
     \AttributeTok{main =} \StringTok{"Linear Predictor (log(μ)) vs. x"}\NormalTok{,}
     \AttributeTok{xlab =} \StringTok{"x"}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"log(μ̂)"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{02-Further_Regression_Methods_files/figure-latex/unnamed-chunk-36-1.pdf}}
...
\item
  Application movement detection with R/Python as example of machine
  learning/AI.
\item
  AI Example, Claude and LEAN with
  \href{https://www.youtube.com/watch?v=zZr54G7ec7A&t=714s&ab_channel=TerenceTao}{Terence}
\item
  Last Chapter - Introduction to AI. Previous statistical analyses is a
  special case of neuronal networks.
\item
  \href{https://onlinelibrary.wiley.com/doi/full/10.1111/cdoe.12617?casa_token=3IzTqt8cjzQAAAAA\%3AhKH7i2yhPi9tIVbOz89JVMT2yuSuyhejGLcJqXVINfgo7kjxlkzzBhqu3iH0zW7ClZef0ivw790enq1I}{table
  2 fallacy}
\item
  mention missing values, missingness mechanisms
\item
  Chapter: Sample size calculations for multivariate regression,
  Proportions, ICCs, t.test
\item
  AIC, BIC, cross-validation, Model selection (best subset,
  leaps\ldots.), Variable selection
\item
  Restrictive cubic splines?
\item
  Causal inference, DAGs, counterfactuals
\item
  \textbf{Final chapter} with ``topics we have not covered for time
  reasons: some known unknowns.'' Topics: Sample size calculations,
  missing data, composition data, causal inference,\ldots{} Frequently
  asked master thesis questions:
