...
The data can be found [here](https://www.cosmin.nl/book-measurement-in-medicine/).
...
- The `chains` argument specifies how many chains we want to run.
  ...
  frequency. [Here](https://mbjoseph.github.io/posts/2018-12-25-animating-the-metropolis-algorithm/) 
...
The **model for ICC 2 and 3** in the `psych` output explicitly considers this
...
The model explains the data better, since it knows about the bias (see [exercise 7](#exercise7)).
...
df %>% 
  ...
  geom_abline(intercept = -SDC, slope = 1, color = "blue", linetype = "dashed") # SDC lower
...
The blue dashed lines show the *Smallest Detectable Change (SDC)*, 
...
This is also true [in the literature](https://www.sciencedirect.com/science/article/pii/S1526590024004899#sec0080).
...
ggplot(df_plot, aes(x, dens)) +
  ...
  annotate("text", x = d_bar,    y = 0,
           label = sprintf("bar(d) == %.2f", d_bar), parse = TRUE,
...
Look at Table 1 in the paper and consider the variable *Plantar flexion MVC (N.m)*.
